...
def unregister_file(story_id: int, episode: int, file_type: str):
    supabase_request(
        "DELETE",
        "files",
        params={
...
async def post_init(application: Application):
    ...
    admin_commands = [
        ...
        BotCommand("unregister", "Remove file registration"),
...
async def unregister_private_command(update, context):
    user = update.effective_user
    ...
    if len(context.args) != 3:
        await update.message.reply_text(
            "❌ Format:\n\n"
            "/unregister STORY_ID EPISODE TYPE\n\n"
            "Example:\n"
            "/unregister 1 2450 video"
        )
        return
    ...
    existing = get_file(story_id, episode, file_type)
    if not existing:
        await update.message.reply_text(
            "❌ File registration nahi mili.\n\n"
            f"Story ID: {story_id}\n"
            f"Episode: {episode}\n"
            f"Type: {file_type}"
        )
    ...
    unregister_file(story_id, episode, file_type)

    await update.message.reply_text(
        "✅ File registration removed successfully.\n\n"
        f"Story ID: {story_id}\n"
        f"Episode: {episode}\n"
        f"Type: {file_type}\n"
        f"Storage Message ID: {existing['storage_message_id']}\n\n"
        "ℹ️ Storage Channel ka original file delete nahi hua hai."
    )
...
async def admin_help(update, context):
    ...
    await update.message.reply_text(
        ...
        "/unregister STORY EP TYPE - Remove file registration\n\n"
